...
The compat.h fails on the line with the contents "#if SIZEOF_OFF_T == 8". And it is absolutely not clear where it actually fails on. Tryning different std:c++ versions did not result in any solution. In the end the #ifdef switch was commented out to let it compile further. But too many compilation errors occurred with the other modules and many of the failures seems to be related to the fact that some c-files are dependent on the optional packages. This switching was normally done by the cmake-system, so I had to go back to get cmake working, such that it would be clear whcich c-files are not dependent on the optional packages, such that only the ZLIB problem remained. Another problem was that the zlib.h (matching the zlib.lib library) needs to include the zconf.h file, but that file is nowhere to be find in the zlib project?!?
